...
AllpassCubic::AllpassCubic() :
    ...
    m_allpass(
        (
            md_audio::AllpassCubic<memory::SCAllocator<double, memory::SCPool>>::set_sample_rate(sampleRate()),
            m_allocator
        ),
        in0(1),
        in0(2),
    ...
    )
{
    if (!m_allpass.initialise()) {
        printf("Could not allocate real-time memory for AllpassCubic\n");

        set_calc_function<AllpassCubic, &AllpassCubic::clear>();
